...
class CustomAnimatedFlipCounter extends StatelessWidget {
  ...
  const CustomAnimatedFlipCounter({
    Key? key,
    required this.value,
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < digits.length - fractionDigits; i++) {
      final digit = _SingleDigitFlipCounter(
        ...
        duration: duration,
        curve: curve,
        size: Size(prototypeDigit.size.width - (Platform.isIOS ? 1.5 : 1.0),
            prototypeDigit.size.height),
        color: color,
      );
